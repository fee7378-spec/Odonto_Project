      ...
      <div className="p-8 space-y-8 animate-in slide-in-from-right duration-500">
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            className="bg-sky-600 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-sky-700 transition-colors shadow-lg shadow-sky-100"
          >
            Novo Agendamento
          </button>
          </div>
            ...
            <div className="medical-card p-6">
              <div className="flex flex-col items-center text-center">
                <img 
                  ...
                  className="w-24 h-24 rounded-2xl object-cover mb-4 ring-4 ring-sky-50 shadow-md"
                />
                <h2 className="text-xl font-bold text-slate-900">{selectedPatient.name}</h2>
                <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full mt-1 uppercase tracking-wider">Paciente Ativo</span>
              </div>
              ...
              <div className="space-y-6">
                {selectedPatient.history.map((item: string, idx: number) => (
                  <div key={idx} className="flex gap-4 relative">
                    ...
                    <div className="w-4 h-4 rounded-full bg-sky-100 border-2 border-sky-600 mt-1 z-10"></div>
                    <div>
                      <p className="text-sm font-bold text-slate-700">{item.split(' - ')[1]}</p>
                      <p className="text-sm text-slate-500 mt-0.5">{item.split(' - ')[0]}</p>
                    </div>
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          onClick={() => alert('Abrir modal de Cadastro de Paciente')}
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full bg-white border border-slate-200 rounded-xl py-3 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20"
          />
        </div>
        <button className="bg-white border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold text-slate-600 flex items-center justify-center gap-2 hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <img 
                      ...
                      className="w-10 h-10 rounded-xl object-cover ring-2 ring-transparent group-hover:ring-sky-200 transition-all" 
                      alt="" 
                    />
                    <div>
                      <p className="text-sm font-bold text-slate-900 group-hover:text-sky-600 transition-colors">{patient.name}</p>
                      <p className="text-[11px] text-slate-400 font-medium">Cadastrado em 12/03/2024</p>
                    </div>
                  </div>
                </td>
